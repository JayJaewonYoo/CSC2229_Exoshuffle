...
for i in range(num_nodes):
    ...
    curr_node.addService(
        pg.Install(
            url="http://www.ordinal.com/try.cgi/gensort-linux-1.5.tar.gz", path="/local"
        )
    )  # The documentation
    curr_node.addService(
        pg.Install(
            url="https://github.com/JayJaewonYoo/CSC2229_Exoshuffle/raw/main/startup_files.tar.gz",
            path="/local",
        )
    )  # The documentation
    ...
    curr_node.addService(
        pg.Execute(
            shell="sh",
            command="/local/startup_script.sh " + str(start) + " " + str(end),
        )
    )
